...
When the cross-cluster link is severed (delete the **`onprem-link-token`** `Link` CR — **not just the Secret**, see critical note below — in `banking-infra` on cloud):
...
The dashboard's "Traffic & Chaos" page exercises this for real via `LinkFailurePanel.tsx` → `PUT /api/backend/link/break|restore` (dashboard-backend) → `PUT /api/gateway/link/break|restore` (cloud `cluster-gateway`, `LinkResource.java`, fabric8 `KubernetesClient` + `GenericKubernetesResource` for the `skupper.io/v2alpha1` `Link` CRD). `cluster-gateway` caches both the live `Link` object and the Secret in memory immediately before deleting them (Link first, so the connection drops immediately) and recreates both verbatim on restore (Secret first, so the Link's `tlsCredentials` reference resolves right away); if its pod restarts in between, the cache is lost and restore returns HTTP 409 (`no-cached-secret`) — the documented manual fallback is re-running the AccessGrant/AccessToken exchange (`bootstrap-phase1.sh` Step 6). RBAC for this (`get`/`delete`/`create` on the named secret and the named `links.skupper.io` resource, namespace `banking-infra`) is added only in `app-services/cluster-gateway/overlays/cloud/rbac-link.yaml` — onprem never gets it, since neither object exists there.

**CRITICAL — confirmed live: deleting only the Secret has NO effect on an already-established link; deleting the `Link` CR is what actually severs it.** The first implementation of this feature deleted only the `onprem-link-token` Secret, exactly as this file's prose (and `bootstrap-phase1.sh`'s original chaos-scenario framing) assumed. Live testing showed the "Interconnect Broken" toggle had **zero observable effect**: `oc get link onprem-link-token -n banking-infra --context cloud` still reported `Ready` / `SITES IN NETWORK: 2` minutes after the Secret was deleted, `skstat -c` on the cloud `skupper-router` showed the `inter-router` connection still fully alive (TLSv1.3, x.509, unaffected uptime), and the cloud processor kept writing to onprem PostgreSQL/Apicurio through the tunnel with `rejectedTotal` staying at 0. Reason: Skupper's router authenticates and opens the inter-router AMQP/TLS session once, using cert material already loaded from the Secret at connection time — it does not watch the Secret for deletion and does not tear down a live session just because the backing object disappears. The `Link` CR is the actual desired-state object the site controller reconciles against: deleting **it** (`oc delete link onprem-link-token -n banking-infra --context cloud`) is confirmed to immediately drop the `inter-router` connection from `skstat -c` and flip `SITES IN NETWORK` from 2 to 1, at which point the cloud processor's logs show real `Connection refused` errors to both `postgresql-primary` and `apicurio-registry` (also RHSI-tunneled) — the actual documented chaos effect. `LinkResource.java` now deletes/restores both objects together.
