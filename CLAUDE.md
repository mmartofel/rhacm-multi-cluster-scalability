...
2. **`transaction-generator` looked like "Kafka is inaccessible" but the broker was fine — the real blocker was Apicurio, and `find-latest` turned it into a permanent hang.** `KAFKA_BOOTSTRAP_SERVERS` correctly points at the local cloud Kafka broker, unaffected by the Listener break. The actual cause was `apicurio.registry.find-latest=true` on the generator's outgoing channel (the only place in the codebase this was set) — it forces an Apicurio round-trip on *every* send instead of caching after the first. Confirmed by decompiling the actual Apicurio Vert.x REST client (`apicurio-common-rest-client-vertx-0.1.18.Final`, `VertxHttpClient.createClientOptions`): it exposes **no** connect/read/idle timeout option in this library version — only TLS settings — so a cut Apicurio connection made every `send()` hang indefinitely, exhausting `max-in-flight` and producing continuous `SRMSG00034: Insufficient downstream requests to emit item`. Fixed by dropping `find-latest` (this demo uses one fixed schema per topic for its whole life, so nothing about which schema is used changes) — the serializer now only needs Apicurio once, making ongoing generation resilient to a later outage. **Residual limitation, accepted, not fixed**: if the generator's pod restarts *while* the link is already broken (cold cache, no prior successful send), its first send still hangs — there's no timeout knob available in this client to bound that. `transaction-processor`/`ledger-service`'s (de)serializers don't set `find-latest` and cache after first resolution, so they aren't vulnerable to the same *every-message* hang — but they share the same cold-cache-during-outage edge case.
